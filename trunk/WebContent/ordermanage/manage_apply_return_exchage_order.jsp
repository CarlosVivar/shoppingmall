...
<%
	...
	List<Productorderdetail> lst=(List<Productorderdetail>)request.getAttribute("listordershippingcompleted");
	if(lst==null){
		getServletContext().getRequestDispatcher("/ManageApplyREOrderController").forward(request, response);
	}
...
<script type="text/javascript">
	...
	function showLinkCancel(check) {
		...
		row = cell.parentNode;
		var rows = document.getElementById('tab3_table').getElementsByTagName(
				'tr');
		//xoa cancel
		for ( var i = 1; i < rows.length; i++) {
			...
			if (radio[0].checked) {
				...
				as[0].innerHTML = "<%=LanguegeBUS.getValue("return_exchange",lang)%>";
			} else {
				as[0].innerHTML = "";
			}
		}
	...
	function loadApplyReturnExchangeOrder(){
		location.replace("<%=ServletUtils.getBaseUrl(request)%>/returnexchangeorder.html");
	}
</script>
...
<body>
												...
												<DIV class="corner-right-bot">
													<DIV class="category-title">
														<ul class="tab" style="padding: 5">
															...
															<li><a rel="tabs3" href="" class="selected"><span><%=LanguegeBUS.getValue("return_exchange", lang)%></span>
															</a></li>
														...
														<div class="tube">
															<div>
																<form action="<%=ServletUtils.getBaseUrl(request)%>/ManageApplyREOrderController" method="post">
																...
																<tbody>
																	<tr>
																		<th width="15%" class="select"><%=LanguegeBUS.getValue("return_exchange", lang)%></th>
																		<td width="85%" class="select"><input type="radio"
																			id="rdo_cancel_1" name="rdo_cancel" checked="checked"
																			 /><label
																			for="rdo_cancel_1" style="margin-right: 40px;"><%=LanguegeBUS.getValue("apply_return_exchange", lang)%>
																				</label> <input type="radio" onclick="loadApplyReturnExchangeOrder()"
																			id="rdo_cancel_2" name="rdo_cancel" 
																			 /><label
																			for="rdo_cancel_2"><%=LanguegeBUS.getValue("returning_exchanging", lang)%></label>
																		</td>
																	</tr>
																	<tr>
																																			...
																																			if (lst != null) {
																																				for (Productorderdetail productODetail : lst) {
																																					...
																																					String deliverCost = DeliverCostBUS.getDeliverCostByProduct(
																																							product, price*quantiy,prod.getTransportId(), lang) + "";
 ...
 %>
																			...
																			<td align="center" ><a 
																				href="returnexchange.html?podid=<%=pODetailID%>"
																				style=""></a></td>
																		</tr>
																		<%
																			}
...
<tr>
                                    	<td colspan="8">
                                    			<%                                   			
					                              ...
					                              <span><a href="applyreturnexchangeorder.html?page=<%=i%>" ><%=i%></a></span>
					                              <%}else{ %>	
