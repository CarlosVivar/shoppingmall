...
<%
	...
	if(lst==null){
		getServletContext().getRequestDispatcher("/ManageOrderApplyCancelController").forward(request, response);
	}
...
<script type="text/javascript">
	...
	function showLinkCancel(check) {
		...
		for ( var i = 1; i < rows.length; i++) {
			...
			if (radio[0].checked) {
				...
				as[0].innerHTML = "<%=LanguegeBUS.getValue("cancel",lang)%>";
			} else {
				as[0].innerHTML = "";
			}
		}
	...
	function loadCancellingorder(){
		var url="<%=ServletUtils.getBaseUrl(request)%>";
		location.replace(url+"/ordercancel.html");
	}
</script>
...
<body>
											...
											<DIV class="corner-left-bot">
												<DIV class="corner-right-bot">
													<DIV class="category-title">
														...
														<div class="tube">
															<div>
																<form action="<%=ServletUtils.getBaseUrl(request)%>/ManageOrderApplyCancelController" method="post">
																...
																<tbody>
																	<tr>
																		...
																		<td width="85%" class="select"><input type="radio"
																			id="rdo_cancel_1" name="rdo_cancel" checked="checked"
																			 /><label
																			for="rdo_cancel_1" style="margin-right: 40px;"><%=LanguegeBUS.getValue("apply_for_cancel", lang)%>
																				</label> <input type="radio"  onclick="loadCancellingorder()"
																			id="rdo_cancel_2" name="rdo_cancel" value="df"
																			 /><label for="rdo_cancel_2"><%=LanguegeBUS.getValue("cancelling", lang)%></label>
																		</td>
																	</tr>
																	<tr>
									                ...
									                if(pagedcust!=null && pagedcust.getPageCount()- 1 > 0){
									              %>
					                              <div class="paging grey page_item"  style="text-align: left" align="left">
					                              <div class="wrap">
					                              <div class="pageOn">
					                               <% 
									                if(!pagedcust.isFirstPage()){%>
					                              <span ><a href="orderapplycancel.html?page=0"><%=LanguegeBUS.getValue("first", lang) %></a></span>&nbsp;&nbsp;
					                               <% }
					                               if(pagedcust.hasIndexPrev()){
					                            	   %>
					                            	    <span><a href="orderapplycancel.html?page=<%=pagedcust.getIndexPrev()%>" ><%=LanguegeBUS.getValue("prev.",lang) %></a></span>
					                               <%}
					                              ...
					                              <%}else{ %>	
									               ...
									               if(pagedcust.hasIndexNext()){									            	 
									            	   %>
									            	   <span><a href="orderapplycancel.html?page=<%=pagedcust.getIndexNext()%>" ><%=LanguegeBUS.getValue("next",lang) %></a></span>
									               <%}
									                	if(pagedcust.isLastPage()==false){	 
									                 %>
					                              &nbsp;&nbsp;<span ><a href="orderapplycancel.html?page=<%=(pagedcust.getPageCount()- 1)%>"><%=LanguegeBUS.getValue("last", lang) %></a></span>
					                               <%} 
									                	%>   
					                             ...
					                             %>
																	<table width="100%" class="manageorderTable"
																		id="tab2_table">
																		<tr>
																			<th class="title" width="20%" colspan="2"><%=LanguegeBUS.getValue("orderNo.", lang)%><div>(<%=LanguegeBUS.getValue("oDate", lang)%>)</div>
																																				...
																																				for (Productorderdetail productODetail : lst) {
																																					...
																																					Orderdetailstatus orderDStatus = OrderdetailstatusBUS
																																							.getOrderDetailStatusById(
																																									productODetail.getOrderDetailStatusId(),
																																									lang);
																																					;
																		%>
																		<tr>
																			<td style="border-right: none !important;"><input
																				type="radio" name="radiCheck"
																				value="<%=pODetailID%>"
																				onClick="showLinkCancel(this)" />
																			</td>
																			...
																			<td align="center"><a href="orderdetailinfo.html?id=<%=pODetailID%>"><%=product.getProductName()%></a>
																			</td>
 ...
 %>
																			...
																			<td align="center"><a
																				href="cancel_order.jsp?podid=<%=pODetailID%>"
																				style=""></a></td>
																		</tr>
																		<%
																			}
                                    	...
                                    	<td colspan="8">
                                    			<%                                  			
					                              ...
					                              <span ><a href="orderapplycancel.html?page=0"><%=LanguegeBUS.getValue("first", lang) %></a></span>&nbsp;&nbsp;
					                               ...
					                               if(pagedcust.hasIndexPrev()){
					                            	   %>
					                            	    <span><a href="orderapplycancel.html?page=<%=pagedcust.getIndexPrev()%>" >Prev.</a></span>
					                               <%}
									                	 for(int i=pagedcust.getIndexStart();i<pagedcust.getIndexLast();i++){
									                		 if(i!=pagedcust.getCurrentPage()){    	                
									               %>
					                              ...
					                              <%}else{ %>	
									               ...
									               if(pagedcust.hasIndexNext()){									            	 
									            	   %>
									            	   <span><a href="orderapplycancel.html?page=<%=pagedcust.getIndexNext()%>" >Next</a></span>
									               <%}
									                	if(pagedcust.isLastPage()==false){	 
									                 %>
					                              &nbsp;&nbsp;<span ><a href="orderapplycancel.html?page=<%=(pagedcust.getPageCount()- 1)%>"><%=LanguegeBUS.getValue("last", lang) %></a></span>
