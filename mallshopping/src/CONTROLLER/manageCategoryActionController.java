		...
		try {
			...
			if (session.getAttribute("username") == null
				...
				if (null != act && act.equals("add")) {
					...
					String cateId_new = (String) request
							.getParameter("cateId_new");
					String childId_new = (String) request
							.getParameter("childId_new");
					String subId_new = (String) request
							.getParameter("subId_new");
					if (null != catename && catename.length() > 0
							&& null != cateId_new) {
					...
					if (null != childname && childname.length() > 0
						...
						boolean test = CategoryChildBUS.insertCategoryChild(cc,
								lang);
						if (test) {
							response.sendRedirect("admin/categoryadd.html");
						} else {
					...
					if (null != subname && subname.length() > 0
						...
						boolean test3 = CategorySubBUS.insertCategorySub(cb,
								lang);
						if (test3) {
				...
				if (null != act && act.equals("update")) {
					...
					if (null != cateId) {
						...
						String caPriorities = (String) request
								.getParameter("caPriorities");
						if ("null".equals(caPriorities)
								&& "".equals(caPriorities)) {
							// System.out.println("caPriorities: "+caPriorities);
							caPriorities = "0";
						}
						cate.setPriorities(caPriorities);
					...
					if (null != childId) {
						...
						String caChilPriorities = (String) request
								.getParameter("caChilPriorities");
						if ("null".equals(caChilPriorities)
								&& "".equals(caChilPriorities)) {
							 System.out.println("caChilPriorities: "+caChilPriorities);
							caChilPriorities = "0";
						}
						cc.setPriorities(caChilPriorities);

						boolean test = CategoryChildBUS.updateCategoryChild(cc,
								lang);
						if (test) {
							response.sendRedirect("admin/category.html");
						} else {
					...
					if (null != subId) {
						...
						CategorySub cb = CategorySubBUS.getCategorySub(subId,
								lang);
						cb.setCategorySubName(subname);

						String caSubPriorities = (String) request
								.getParameter("caSubPriorities");
						if ("null".equals(caSubPriorities)
								&& "".equals(caSubPriorities)) {
							 System.out.println("caSubPriorities: "+caSubPriorities);
							caSubPriorities = "0";
						}
						cb.setPriorities(caSubPriorities);

						boolean test3 = CategorySubBUS.updateCategorySub(cb,
								lang);
						if (test3) {
							response.sendRedirect("admin/category.html");
						} else {
