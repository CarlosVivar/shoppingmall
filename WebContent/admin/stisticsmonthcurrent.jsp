...
<%
    Object obj = request.getAttribute("lststisticsMonthCurrent");
    if (obj == null) {
        String url = "/StatisticsAdminMonthCurrentController";
                                ...
                                <div class="box_ucp_inside">
                                    ...
                                    <div id="usual2">
                                        ...
                                        <ul class="tab">
                                            ...
                                            <li><a href="<%=ServletUtils.getBaseUrl(request)%>/admin/salemonthcurrent.html" class="selected"><span><%=LanguegeBUS.getValue("month", lang) %></span></a></li>
                                            <li><a href="<%=ServletUtils.getBaseUrl(request)%>/admin/saleyearcurrent.html" ><span><%=LanguegeBUS.getValue("year", lang) %></span></a></li>
                                        </ul>
                                        <div class="clear"></div>

                                        <form action="<%=ServletUtils.getBaseUrl(request)%>/StatisticsAdminMonthCurrentController" method="post" id="viewForm">
                                            <table style="display: table;" id="tabs1" class="table_ucp" border="0" cellpadding="0" cellspacing="0" width="100%">
                                                <tbody>
                                                    <tr>
                                                        <td colspan="9" class="nonSearch" width="110"><%=LanguegeBUS.getValue("selectmonth", lang) %> &nbsp;
                                                            <select name="selectcbB"  onchange="onViewSubmit()" style="width:50px">
                                                    ...
                                                    <%
                                                        float total = 0;
                                                        List<Productorderdetail> lstProductDetail = (List<Productorderdetail>) request.getAttribute("lststisticsMonthCurrent");
                                              ...
                                              <% if (lstProductDetail != null && !lstProductDetail.isEmpty() ) { %>
                                                    <tr>
				                                    	<td colspan="8">
				                                    			<%NavigationInfo pagedcust = (NavigationInfo)session.getAttribute("pagemonth");
									                              ...
									                              <span ><a href="<%=ServletUtils.getBaseUrl(request)%>/admin/salemonthcurrent.html?selectcbB=<%=(String)session.getAttribute("selectcbB") %>&page=0"><%=LanguegeBUS.getValue("first", lang) %></a></span>
									                               <% }
													                	 for(int i=1;i<pagedcust.getIndexList().length;i++){
													                		 if(i!=pagedcust.getCurrentPage()){    	                
													               %>
									                              <!-- span class="active">1</span-->
									                              <span><a href="<%=ServletUtils.getBaseUrl(request)%>/admin/salemonthcurrent.html?selectcbB=<%=(String)session.getAttribute("selectcbB") %>&page=<%=i%>" ><%=i%></a></span>
									                              <%}else{ %>	
													                 ...
													                 %>
									                              <span ><a href="<%=ServletUtils.getBaseUrl(request)%>/admin/salemonthcurrent.html?selectcbB=<%=(String)session.getAttribute("selectcbB") %>&page=<%=(pagedcust.getPageCount()- 1)%>"><%=LanguegeBUS.getValue("last", lang) %></a></span>
									                               <%} 
													                	%>   
